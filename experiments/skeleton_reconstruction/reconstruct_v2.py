"""V2 — Skeleton-driven mask reconstruction with Steger Hessian integration.

Two additions over v1:

1. **Centerline snap**: each skeleton sample is refined to its sub-pixel
   ridge position via Steger's Taylor-Hessian method. When the snap is
   valid (|t| ≤ max_offset), downstream profile extraction uses the
   refined centre + Hessian-derived normal instead of the raw skeleton
   pixel + PCA-tangent-derived perpendicular.

2. **Hessian-aligned thin-vessel rescue**: the Phase-2 fallback still fits
   a Gaussian (`fit_thin_vessel_gaussian`), but the perpendicular profile
   is sampled along the Hessian normal — more reliable near the resolution
   limit than the PCA-derived perp on a 1-pixel skeleton.

Everything else (Phase-3 interpolation, per-pixel model-threshold paint,
σ-clamp, junction/crossing skip, disc-paint flag) is preserved verbatim
from v1.
"""
...
# V2 addition: Steger Hessian-based sub-pixel ridge refinement
from steger import hessian_components, steger_refine_single  # type: ignore  # noqa: E402
...
class ReconstructCfg:
    ...
    # V2-Steger-specific knobs. `steger_enabled=False` makes v2 behave
    # exactly like v1 so we can A/B by flipping this flag.
    steger_enabled: bool = True
    steger_sigma: float = 1.5                  # Hessian smoothing σ (px)
    steger_max_offset: float = 2.0             # |t| Taylor-refine cap (px)
    steger_dark_ridge: bool = True             # green-channel vessels are dark
...
class ReconstructionStats:
    ...
    n_steger_snapped: int = 0          # V2: samples using Hessian-refined centre
...
def reconstruct_one_class(
    ...
    # V2: precompute Hessian on the green channel ONCE. steger_refine_single
    # then runs in O(1) per skeleton point. σ is configurable; 1.5 px is a
    # reasonable compromise for CFP vessels spanning 1-15 px diameters.
    hess = None
    if cfg.steger_enabled:
        hess = hessian_components(green, sigma=cfg.steger_sigma)
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        for s in sample_positions:
            ...
            # --- V2 centerline refinement ---
            # PCA tangent is always the fallback (robust on fragmented
            # skeletons and the junction-adjacent samples that Steger
            # rejects). Steger replaces it when the Taylor snap converges.
            tangent = tangent_pca(pts, j, window=7)
            perp = perpendicular(tangent)
            cy, cx = float(pts[j, 0]), float(pts[j, 1])
            steger_valid = False
            if hess is not None:
                ry, rx, hess_normal, strength, ok = steger_refine_single(
                    y_i, x_i, hess,
                    dark_ridge=cfg.steger_dark_ridge,
                    max_offset=cfg.steger_max_offset,
                )
                if ok:
                    cy, cx = ry, rx
                    perp = np.asarray(hess_normal, dtype=float)
                    steger_valid = True
                    stats.n_steger_snapped += 1
